...
router.get("/requests/all", async (req, res) => {
  try {
    ...
    const employees = await Employee.find(employeeQuery).select(
      "name email department designation leaveRequests"
    );
    ...
    allRequests.sort(
      (a, b) =>
        new Date(b.appliedOn || b.createdAt) -
        new Date(a.appliedOn || a.createdAt)
    );
...
router.put("/request/:requestId/status", async (req, res) => {
  try {
    ...
    if (status === "approved") {
      ...
      switch (leaveType) {
        case "casual":
          employee.casualLeaves = Math.max(
            0,
            (employee.casualLeaves || 1) - duration
          );
          break;
        case "sick":
          employee.sickLeaves = Math.max(
            0,
            (employee.sickLeaves || 2) - duration
          );
          break;
        case "earned":
          employee.earnedLeaves = Math.max(
            0,
            (employee.earnedLeaves || 1) - duration
          );
          break;
        case "unpaid":
  ...
  try {
    ...
    if (department && department !== "all") {
      pendingRequests = pendingRequests.filter(
        (req) => req.employeeId && req.employeeId.department === department
      );
    }
...
router.get("/on-leave-today", async (req, res) => {
  try {
    ...
    const formattedData = [];
    employeesOnLeave.forEach((employee) => {
      ...
      if (activeLeave) {
        formattedData.push({
          ...
          duration:
            activeLeave.duration ||
            calculateDuration(activeLeave.startDate, activeLeave.endDate),
